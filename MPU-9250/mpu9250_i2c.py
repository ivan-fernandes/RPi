...
# function to read accel and gyro values
def MPU6050_start():
...
##    t_val = read_raw_bits(TEMP_OUT_H) # uncomment to read temp
    ...
    a_x = (acc_x/(2.0**15.0))*accel
    a_y = (acc_y/(2.0**15.0))*accel
    a_z = (acc_z/(2.0**15.0))*accel

    w_x = (gyro_x/(2.0**15.0))*gyro
    w_y = (gyro_y/(2.0**15.0))*gyro
    w_z = (gyro_z/(2.0**15.0))*gyro
...
# MPU6050 Registers (gyroscope and accelerometer)
MPU6050_ADDR = 0x68
PWR_MGMT_1   = 0x6B
SMPLRT_DIV   = 0x19
CONFIG       = 0x1A
...
bus = smbus.SMBus(1)
# instantiate gyro and accel values
gyro, accel = MPU6050_start()
# AK8963_start()   # uncomment if EDA and ECL are connected at the same pins as SDA and SCL (pins 5 (GPIO3) and 3 (GPIO2), respectively)
